...
<section class="max-w-7xl mx-auto px-4 py-12">
  ...
  <?php
    ...
    $per = 4;
    $file = __DIR__ . '/../data/portfolios/portfolio_' . ($tab==='mobile'?'mobile':'website') . '.json';
  ...
  <div class="flex gap-3 mb-6">
    ...
    <a class="px-4 py-2 rounded-full border <?= $tab==='mobile'?'bg-cyan-600 text-white border-cyan-600':'border-gray-300' ?>" href="?tab=mobile">Mobile App</a>
  </div>
  <?php if(!$slice): ?>
    ...
    <div class="grid md:grid-cols-2 gap-6">
      <?php foreach($slice as $it): ?>
        <div class="border border-gray-200 rounded-xl p-5">
          <?php if(!empty($it['image'])): ?><img class="rounded-lg mb-4" src="/uploads/<?= htmlspecialchars($it['image']) ?>" alt=""><?php endif; ?>
          <h3 class="text-xl font-semibold mb-1"><?= htmlspecialchars($it['title']) ?></h3>
          <p class="text-gray-600 mb-2"><?= htmlspecialchars($it['description']) ?></p>
          <?php if(!empty($it['url'])): ?><a class="text-cyan-700" href="<?= htmlspecialchars($it['url']) ?>" target="_blank">Visit</a><?php endif; ?>
